...
describe("release artifact policy", () => {
  ...
  it("provides a manual macOS signing diagnostics workflow without release uploads", async () => {
    ...
    expect(workflow).toContain("Unlock macOS keychain");
    expect(workflow).toContain("MAC_KEYCHAIN_PASSWORD: ${{ secrets.MAC_KEYCHAIN_PASSWORD }}");
    expect(workflow).toContain("MAC_KEYCHAIN_PASSWORD secret is missing.");
    expect(workflow).toContain("security unlock-keychain -p \"$MAC_KEYCHAIN_PASSWORD\" \"$LOGIN_KEYCHAIN\"");
    expect(workflow).toContain("security set-key-partition-list -S apple-tool:,apple:,codesign:");
    expect(workflow).toContain("security default-keychain -s \"$LOGIN_KEYCHAIN\"");
    expect(workflow).toContain("security find-identity -v -p codesigning");
    expect(workflow).toContain("head -n 20 diagnostics/notary-profile-history.txt");
    ...
    expect(workflow).toContain("macos-build-diagnostics-0.2.6");
